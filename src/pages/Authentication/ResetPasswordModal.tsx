...
const ResetPasswordModal: React.FC<ResetPasswordModalProps> = ({ email, onClose, onSuccess }) => {
  ...
  const handleVerify = async (code: string) => {
    ...
    try {
      ...
      setSuccess(language === 'en'?'Reset Password Successfully!':'重置密码成功！');
      onSuccess(); // Notify parent component of successful password reset
    } catch (error) {
      setError(language === 'en'
    ? `Error resetting password: ${error.message}`
    : `重置密码错误：${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full relative ">
        ...
        {language === 'en'? 'Reset Your Password':'重置您的密码'}
        </h2>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <div className="mb-4">
            <label htmlFor="newPassword" className="block text-sm text-gray-600 mb-2 text-center">
              {language === 'en'? 'Enter your new password:':'输入您的新密码'}
            </label>
            <input
              type="password"
              id="newPassword"
          ...
          <div className="mb-4">
            <label htmlFor="verificationCode" className="block text-sm text-gray-600 mb-2 text-center">
              {language === 'en'? 'Enter the 6-digit code sent to your email:':'输入您邮箱收到的6位验证码:'}
            </label>
