...
import {setSessionVariableWithExpiration, getSessionVariableWithExpiration, fetchAndStoreUserData} from '../../components/sessionOperation.tsx'
...
//   const {reFetchUserInfo, reFetchUserStripe} = useStripeStatus(); // Use the hook at the top level of your component
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.nextStep && user.nextStep.signInStep === 'CONFIRM_SIGN_UP') {
        setError(language === 'en' ? "Your email hasn't been verified. Please check your email for verification instructions.": "您的电子邮件尚未经过验证。请检查您的电子邮件以获取验证说明。");
        setIsModalOpen(true); // Open the verification modal
      } else {
//           await reFetchUserInfo(); // Re-fetch the user info after successful sign-in
//           await reFetchUserStripe()
          setSuccess(language === 'en' ? 'Sign-in successful!': "登录成功！");
          ...
          if (err.name === 'NotAuthorizedException') {
            setError(language === 'en' ?'Incorrect Email or Password. Please Try Again.': '登录邮箱或密码错误。请重试。');
          } else if (err.name === 'UserAlreadyAuthenticatedException') {
              setSuccess(language === 'en' ? 'You have already Signed in. Directing into the tool...': '你已经登录平台。跳转中... ');
              await fetchAndStoreUserData()
              setError('');
              navigate('/dashboard');
          }
  ...
  return (
      <div className="h-screen w-screen flex items-center bg-white justify-center bg-gray-100 ">
