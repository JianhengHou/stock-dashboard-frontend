...
import {setSessionVariableWithExpiration, getSessionVariableWithExpiration} from '../../components/sessionOperation.tsx'
...
const DropdownUser = () => {
  ...
  const {stripeCustomerId, preferredName, email} = cognito_data
  ...
  const handleSignOut = async () => {
    try {
      await signOut();
      sessionStorage.removeItem('session_cognify_status'); // Remove the item if it has expired
      sessionStorage.removeItem('session_stripe_status'); // Remove the item if it has expired
      // Optionally, clear any local state or storage if needed
...
//       setEndDate(null)
//       setLoading(true)
      navigate('/auth/signIn'); // Navigate to the sign-in page or another page
  ...
  return (
    <div className="relative">
      ...
      >
        <ul className="flex flex-col gap-5 border-b border-stroke px-5 py-4 dark:border-strokedark">
          <li>
            <Link
              to="/settings"
